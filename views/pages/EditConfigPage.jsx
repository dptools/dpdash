...
class EditConfigPage extends Component {
  ...
  fetchConfigurations = (uid, config) => {
    return window
      .fetch(`${basePath}/api/v1/users/${uid}/configs/${config}`, {
        method: 'GET',
        headers: {
  ...
  reOrganizeRange = (row) => {
    ...
    let max = range[range.length - 1]
      ? bignumber(range[range.length - 1])
      : add(min, 1)
          ...
          >
            ...
            <CardMedia>
              <TextField
                ...
                margin="dense"
                onChange={(event) =>
                  this.handleTextChange(event, item, 'category')
                }
              />
              <TextField
                ...
                margin="dense"
                onChange={(event) =>
                  this.handleTextChange(event, item, 'analysis')
                }
              />
              <TextField
                ...
                margin="dense"
                onChange={(event) =>
                  this.handleTextChange(event, item, 'variable')
                }
              />
              <TextField
                ...
                margin="dense"
                onChange={(event) =>
                  this.handleTextChange(event, item, 'label')
                }
              />
              <div>
                ...
                <Select
                  ...
                  value={
                    item in this.state.themes ? this.state.themes[item] : 0
                  }
              ...
              >
                <TextField
                  ...
                  margin="dense"
                  onChange={(event) =>
                    this.handleRangeChange(event, item, 'min')
                  }
                />
                <TextField
                  value={
                    config[item]['range'][config[item]['range'].length - 1]
                  }
                  label="Max"
                  style={{
                    display: 'table-cell',
                  ...
                  margin="dense"
                  onChange={(event) =>
                    this.handleRangeChange(event, item, 'max')
                  }
                />
              </div>
              <FormControlLabel
                ...
                control={
                  <Checkbox
                    ...
                    onChange={(event, isInputChecked) =>
                      this.makeVisible(event, isInputChecked, item)
                    }
  ...
  saveConfiguration = () => {
    let newReaders = this.state.readers.map((o) => {
      return o.value
    })
    let newConfig = {
      _id: this.state._id,
    ...
    return window
      .fetch(`${basePath}/api/v1/users/${this.state.user.uid}/configs`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  render() {
    ...
    return (
      ...
      >
        <IconButton href={`${basePath}/u/configure/`}>
          <Back color="#5F7B89" />
        </IconButton>
        <TextField
...
export default withRouter(EditConfigPage)
