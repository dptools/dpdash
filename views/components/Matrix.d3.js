...
export default class Matrix extends AbstractGraph {
  ...
  create = (data) => {
    ...
    this.lastDayForFilter =
      this.props.lastDay === null || this.props.lastDay === ''
        ? this.props.maxDay
        : this.props.lastDay
    let category = null
    this.cards = this.svg
    ...
    for (let type = 0; type < data.length; type++) {
      ...
      let range =
        data[type].range.length == 2
          ? d3.range(
              data[type].range[0],
              data[type].range[1],
              (data[type].range[1] - data[type].range[0]) / 8
            )
          : data[type].range
      let thresholdScale = d3.scaleThreshold().domain(range).range(colors)
      ...
      if (data[type] && data[type].hasOwnProperty('data')) {
        ...
        card
          ...
          .filter((d) => {
            return (
              d.day >= this.startDayForFilter && d.day <= this.lastDayForFilter
            )
          })
          .attr('x', (d, i) => {
            return (d.day - this.startDayForFilter) * cellWidth
          })
          .attr('y', (d, i) => {
            return type * this.cardSize
          })
          ...
          .style('fill', (d) => {
            if (
              d[data[type].variable] !== '' &&
              d[data[type].variable] !== undefined
            ) {
        ...
        if (data[type]['text'] == true) {
          card
            ...
            .filter((d) => {
              return (
                d.day >= this.startDayForFilter &&
                d.day <= this.lastDayForFilter
              )
            })
            .text((d) => {
              return d[data[type].variable]
            ...
            .attr('x', (d, i) => {
              return (
                (d.day - this.startDayForFilter) * cellWidth + cellWidth / 2
              )
            })
            .attr('y', (d, i) => {
              return type * this.cardSize + (this.cardSize * 3) / 4
            })
    ...
    let leftMarginWhiteOut = this.generateMarginWhiteOutLeft(
      this.matrixHeight,
      this.maxYAxisWidth,
      margin.left
    )
    ...
    this.categoryMarker = this.generateCategoryMarker(
      this.matrixWidth,
      this.matrixHeight,
      this.maxYAxisWidth,
      yIndices
    ...
    this.xAxisForDatesData = this.generateXAxisForDatesData(
      this.startDayForFilter,
      this.lastDayForFilter,
      this.props.consentDate
    )
    this.generateXAxis(
      this.height,
      this.matrixHeight,
      this.xAxisForDatesData,
      this.startDayForFilter,
      this.lastDayForFilter
    )
  ...
  zoomed = () => {
    ...
    this.categoryMarker.attr(
      'transform',
      'translate(' +
        (eventTransform.x - this.maxYAxisWidth) +
        ',' +
        eventTransform.y +
        ') scale(' +
        eventTransform.k +
    ...
    this.yAxisLeft.call(
      this.yAxisLinear.scale(eventTransform.rescaleY(this.yScaleLinear))
    )
    this.yAxisLeft
      .selectAll('text')
      .data(this.data)
    ...
    this.xAxisBottom.call(
      this.xAxisLinearBottom.scale(
        eventTransform.rescaleX(this.xScaleLinearBottom)
      )
    )
    this.xAxisTop.call(
      this.xAxisLinearTop.scale(eventTransform.rescaleX(this.xScaleLinearTop))
    )
    this.xAxisTop
      .selectAll('text')
      .data(
        this.xAxisForDatesData.filter((d, i) => {
          return (
            d.day >= this.startDayForFilter && d.day <= this.lastDayForFilter
          )
        })
      ...
      .attr('class', (d, i) => {
        return d.marker == 'N/A' ? 'EmptyDates' : 'DatesText'
      })
    // }
  }
  pan = () => {
    ...
    let transformAttrArray = transformAttr
    ...
    this.yAxisLeft.call(
      this.yAxisLinear.scale(transformation.rescaleY(this.yScaleLinear))
    )
    this.yAxisLeft
      .selectAll('text')
      .data(this.data)
    ...
    this.xAxisBottom.call(
      this.xAxisLinearBottom.scale(
        transformation.rescaleX(this.xScaleLinearBottom)
      )
    )
    this.xAxisTop.call(
      this.xAxisLinearTop.scale(transformation.rescaleX(this.xScaleLinearTop))
    )
    this.xAxisTop
      .selectAll('text')
      .data(
  ...
  generateXAxis = (
    ...
    let xAxisBottomYCoordinate =
      pageHeight > 0 && pageHeight < matrixHeight
        ? pageHeight - this.cardSize * 2 + 1
    ...
    this.xScaleLinearBottom = d3
      ...
      .domain([
        startDayForFilter,
        xAxisForDatesData[xAxisForDatesData.length - 1].day,
      ])
    ...
    if (xAxisForDatesData.length > 0) {
      ...
      this.xAxisBottom = this.svg
        ...
        .attr('transform', (d, i) => {
          return 'translate(0,' + xAxisBottomYCoordinate + ')'
        })
    ...
    this.xAxisTop
      .selectAll('text')
      .data(
        xAxisForDatesData.filter((d, i) => {
          return d.day >= startDayForFilter && d.day <= lastDayForFilter
        })
      )
      .text((d) => {
        return d.marker
      ...
      .style('fill', (d, i) => {
        return d.marker == 'N/A' ? '#ffffff' : '#545454'
      })
      .on('click', (d, i) => {
        let day = d.day
        let link =
          '/deepdive/' + this.props.study + '/' + this.props.subject + '/' + day
  ...
  generatePlotDataButton = (
    ...
    button
      ...
      .style('fill', (d, i) => {
        return this.props.width !== 0 ? '#ffffff' : 'none'
      })
      .attr('class', 'Button')
    button
      .append('text')
  ...
  generateMarginWhiteOutBottom = (
    ...
    let bottomYCoordinate =
      pageHeight > 0 && pageHeight < matrixHeight
        ? pageHeight - this.cardSize * 2 + 1
        : matrixHeight - this.halfCardSize + 1
    let marginWhiteOut = this.svg
      .append('g')
